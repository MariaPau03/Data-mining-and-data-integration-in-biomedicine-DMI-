...
```{r}
#In the heatmap, each row typically represents a single patient (sample) and each column represents a biomarker (wbc, alt, glucose, ...) The intensity of the color indicates the elvel of the biomarker!
#As biomarkers have very different units, units must be scaled so all of them are compared ona relative basis. 
#by grouping by group, allows to see if "severe" patients hve a distinct color pattern compared to the "healthy" controls
...
mydata <- mydata %>%
  # Create a group column inside mydata dataframe
  mutate(
    group = case_when(
      group_d %in% c(0, 1) ~ "Non-COVID-19",
      group_d == 2 ~ "Non-severe",
...
heatmap_data <- mydata %>% 
  #select ID, group, sex and ordered biomarkers
  select(patient_id_a, group, sex_g, all_of(ordered_vars)) %>%
  #Remove rows with missing values 
  drop_na() %>%
  #Turn patient_id into row names
  column_to_rownames("patient_id_a")

#CREATE ANNOTATION DATAFRAME:
annotation_col <- heatmap_data %>% 
  select(group, sex_g) %>%
  rename(Group = group, Gender = sex_g)
  
#we must exclude "group" and "sex_g" from the matrix
matrix_to_plot <- heatmap_data %>%
  select(all_of(ordered_vars))


matrix_to_plot$crp_i_mg_l_2<- as.numeric(matrix_to_plot$crp_i_mg_l)

matrix_to_plot$crp_i_mg_l<- gsub("/", NA, matrix_to_plot$crp_i_mg_l)
matrix_to_plot$crp_i_mg_l<- gsub("<1.3", NA, matrix_to_plot$crp_i_mg_l)
matrix_to_plot <- matrix_to_plot %>% select(-crp_i_mg_l_2)
matrix_to_plot$crp_i_mg_l<- as.numeric(matrix_to_plot$crp_i_mg_l)
# Restore row names (Patient IDs) because the apply() function strips them
rownames(matrix_to_plot) <- rownames(heatmap_data)

#?apply()

h <- pheatmap(
  t(matrix_to_plot),           # Transpose so biomarkers are rows and patients are columns
  scale = "row",              # Normalizes data (Z-score) so markers are comparable
